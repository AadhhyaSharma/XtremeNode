                ...
                <div class="min-w-0 space-y-1">
                  ...
                  <p class="text-xs opacity-80 m-0">
                    {{
                      ...
                      'XtremeNode detected a recent crash dump. Please export a crash bundle and include it when filing an issue.'
                    }}
                  </p>
                ...
                <div class="min-w-0">
                  ...
                  <p class="text-xs opacity-80 m-0">
                    {{
                      ...
                      'XtremeNode requires the ViGEmBus driver to emulate controllers on Windows. It is no longer bundled. Please download and install it manually:'
                    }}
...
import XtremeNodeVersion, { GitHubRelease } from '@/sunshine_version';
...
const installedVersion = ref<XtremeNodeVersion>(new XtremeNodeVersion('0.0.0'));
...
const githubVersion = computed(() =>
  githubRelease.value
    ? XtremeNodeVersion.fromRelease(githubRelease.value)
    : new XtremeNodeVersion('0.0.0'),
);
const preReleaseVersion = computed(() =>
  preReleaseRelease.value
    ? XtremeNodeVersion.fromRelease(preReleaseRelease.value)
    : new XtremeNodeVersion('0.0.0'),
);
...
async function runVersionChecks() {
  ...
  try {
    ...
    installedVersion.value = new XtremeNodeVersion(serverVersion || '0.0.0');
    ...
    try {
      githubRelease.value = await fetch(
        'https://api.github.com/repos/Nonary/XtremeNode/releases/latest',
      ).then((r) => r.json());
    } catch (e) {
    ...
    try {
      const releases = await fetch('https://api.github.com/repos/Nonary/XtremeNode/releases').then(
        (r) => r.json(),
      );
      if (Array.isArray(releases)) {
        ...
        if (prereleases.length > 0) {
          ...
          let bestV = XtremeNodeVersion.fromRelease(best);
          for (let i = 1; i < prereleases.length; i++) {
            const cand = prereleases[i];
            const candV = XtremeNodeVersion.fromRelease(cand);
    ...
    // Tag-based comparison handled below via XtremeNodeVersion
